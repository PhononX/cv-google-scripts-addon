function getHeaderButtonSet(card) {
  if (card === 'voiceMemosCard') {
    return CardService.newButtonSet()
      ...
      .addButton(
        getActiveButton('Voice Memos', 'updateVoiceMemosCardHeaderButton', voiceMemoCWIconUrl)
      );
    ...
    return CardService.newButtonSet()
      ...
      .addButton(
        getAnotherButton('Voice Memos', 'updateVoiceMemosCardHeaderButton', voiceMemoCV400IconUrl)
      );
  } else {
...
function getAnotherButton(text, functionName, iconUrl) {
  return CardService.newTextButton().setText('<font color=#5E4CCE><b>' + text + '</b></font>')
    ...
    .setIconUrl(iconUrl)
    // .setOnClickAction(CardService.newAction().setFunctionName(functionName).setParameters({ cardNavigationType: 'update'}));
