function showSettingsCard(e) {
  ...
  return CardService.newNavigation().pushCard(settingsCard());
}
...
function settingsCard(e) {
  ...
  const timeZones = ['UTC', 'Africa/Abidjan',
    'Africa/Accra',
    'Africa/Addis_Ababa',
    'Africa/Algiers',
    'Africa/Asmara',
    ...
    'America/Arrgentina/Rio_Gagentina/Mendoza',
    'America/Allegos',
  ...
  const {hasAccess, authUrl, firstName, lastName, emailTxt, imageUrl} = checkWhoAmI();
  if (hasAccess === true) {
    section.addWidget(CardService.newDecoratedText()
      .setText(firstName + ' ' + lastName)
      .setBottomLabel(emailTxt)
      .setStartIcon(CardService.newIconImage()
        .setIconUrl(imageUrl)
        .setImageCropType(CardService.ImageCropType.CIRCLE)));

    logInLogOutButton = CardService.newTextButton().setText('Sign Out of Carbon Voice').setOnClickAction(CardService.newAction().setFunctionName('signOutOfCarbonVoice'));

  } else {
    section.addWidget(CardService.newTextParagraph().setText('Logged out'))
    const action = CardService.newAuthorizationAction()
      .setAuthorizationUrl(authUrl);

    logInLogOutButton = CardService.newTextButton().setBackgroundColor('#5E4CCE')
      .setText('Sign in with Carbon Voice').setTextButtonStyle(CardService.TextButtonStyle.FILLED)
      .setAuthorizationAction(action);
  }

  section.addWidget(logInLogOutButton);
...
function signOutOfCarbonVoice(e) {
  reset();
  return CardService.newActionResponseBuilder()
    .setStateChanged(true)
    .setNavigation(CardService.newNavigation()
      .updateCard(settingsCard()))
    .setNotification(CardService.newNotification()
      .setText('Successfully signed out!'))
    .build();
}
...
function handleCheckboxChange(e) {
  ...
  return CardService.newActionResponseBuilder()
    .setNotification(CardService.newNotification()
      .setText('The "Keep me signed in" setting was successfully changed.'))
    .build();
}
...
function checkWhoAmI() {
    ...
    const firstName = result?.json?.user?.first_name;
    const lastName = result?.json?.user?.last_name;
    const emailTxt = result?.json?.user?.email_txt;
    const imageUrl = result?.json?.user?.image_url;

    return { hasAccess: true, firstName, lastName, emailTxt, imageUrl };
}